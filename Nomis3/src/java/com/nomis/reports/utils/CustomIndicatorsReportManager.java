...
public class CustomIndicatorsReportManager 
{
    ...
    public void processCustomIndicatorsByLevel2Ou(ReportParameterTemplate rpt,List selectedIndicators,int startMth, int startYear,int endMth,int endYear,String currentUser)
    {        
        ...
        for(int i=0; i<selectedIndicators.size(); i++)
        {
            ...
            {//OVC_BIRTHCERT getBirthRegistrationAcquisitionService()
                processsOVCWithBirthCertificateServed(rpt,partnerCode,AppConstant.SAFETY_DOMAIN,OvcServiceAttributesManager.getBirthRegistrationAcquisitionService().getServiceCode());
            }
    ...
    public void processsOVCWithBirthCertificateServed(ReportParameterTemplate rpt,String partnerCode,int serviceDomain,String serviceType)
    {
        ...
        CustomIndicatorsReport numeratorCir=getDatimEquivalentDataForActiveForExactReportPeriod(rpt,indicatorCode,partnerCode,AppConstant.ACTIVE_NUM,AppConstant.ALL_SERVICE_DOMAIN,null,null,true,0,AppConstant.CHILD_HAS_BIRTHCERTIFICATE,0);
                //processDataByCBO(rpt,indicatorCode,partnerCode);
    ...
    public void processsNumberOfActiveHouseholdsWhoseOvcWereServed(ReportParameterTemplate rpt,String partnerCode)
    {
        ...
        getDatimEquivalentDataForOVC_HHGRAD(rpt,indicatorCode,partnerCode);
        //processDataByCBO(rpt,indicatorCode,partnerCode);
    }
    public void processsNumberOfGraduatedHouseholdsWhoseOvcWereServed(ReportParameterTemplate rpt,String partnerCode)
    {
        String indicatorCode=ind.getIndicatorForNumberOfGraduatedHouseholdsWhoseOvcWereServedWithinReportingPeriod().getIndicatorId();
        processDataByCBO(rpt,indicatorCode,partnerCode);
    }
            ...
            for(int j=0; j<ageDisaggregation.length; j+=2)
            {
                ...
                else
                {
                    //Enrollment stream indicators are only for children less than 18
                        ...
                        {
                            //DatimReportTemplate drt=drg.getNoOfActiveCaregiversServed(rpt, rpt.getStartDate(),rpt.getEndDate(),rpt.getStartAge(),rpt.getEndAge(),male);
                             maleCount=drg.getNoOfActiveCaregiversServedByEnrollmentStatus(rpt,rpt.getStartDate(),rpt.getEndDate(),rpt.getStartAge(),rpt.getEndAge(),male);
                             femaleCount=drg.getNoOfActiveCaregiversServedByEnrollmentStatus(rpt,rpt.getStartDate(),rpt.getEndDate(),rpt.getStartAge(),rpt.getEndAge(),female);
    ...
    private CustomIndicatorsReport getDatimEquivalentDataForOVC_HHGRAD(ReportParameterTemplate rpt,String indicatorCode,String partnerCode)
    {
        CustomIndicatorsReport cir=null;
        ...
        try
        {
            ...
            int[] ageDisaggregation=hhgradAgeDisaggregation; //ovcAgeDisaggregation;
            ...
            for(int j=0; j<ageDisaggregation.length; j+=2)
            {
                ...
                //rt=orm.getOVC_HHGRADForDatim(rpt,AppConstant.ACTIVE_NUM);
                activeMaleCount=rt.getMaleTotal();
                activeFemaleCount=rt.getFemaleTotal();
                
                //rt=orm.getOVC_HHGRADForDatim(rpt,AppConstant.GRADUATED_NUM);
                graduatedMaleCount=rt.getMaleTotal();
    ...
    private CustomIndicatorsReport getDatimEquivalentDataForOVC_HTSLINK(ReportParameterTemplate rpt,String indicatorCode,String partnerCode,int childAtRisk,int referralComplete,int hivStatus,int enrolledOnTreatment)
    {
        ...
        try
        {
            ...
            for(int j=0; j<ageDisaggregation.length; j+=2)
            {
                ...
                if(rpt.getStartAge()<18)
                {
                     ReportTemplate activeRt=orm.getNumberOfChildrenAssessedForHivRiskAndReferredForHivTestForTreatmentCascade(rpt,startDateOfQuarter,rpt.getEndDate(),childAtRisk,referralComplete,hivStatus,enrolledOnTreatment,AppConstant.ACTIVE_NUM);
                     ReportTemplate graduatedRt=orm.getNumberOfChildrenAssessedForHivRiskAndReferredForHivTestForTreatmentCascade(rpt,startOfFinancialYear,rpt.getEndDate(),childAtRisk,referralComplete,hivStatus,enrolledOnTreatment,AppConstant.ACTIVE_NUM);
